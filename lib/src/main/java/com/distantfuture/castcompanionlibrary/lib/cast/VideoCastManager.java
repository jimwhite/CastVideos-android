...
public class VideoCastManager extends BaseCastManager implements OnMiniControllerChangedListener, OnFailedListener {
   ...
   * @throws TransientNetworkDisconnectionException
   */
  public boolean removeDataChannel() throws TransientNetworkDisconnectionException, NoConnectionException {
    checkConnectivity();
    if (TextUtils.isEmpty(mDataNamespace)) {
      return false;
    }
  ...
  private void updateRemoteControl(boolean playing) {
    ...
    try {
      ...
      if (mRemoteControlClientCompat != null) {
        mRemoteControlClientCompat.setPlaybackState(playing ? RemoteControlClient.PLAYSTATE_PLAYING : RemoteControlClient.PLAYSTATE_PAUSED);
      }
    } catch (TransientNetworkDisconnectionException e) {
